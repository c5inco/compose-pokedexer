...
/**
 * Composable helper to get a ViewModel from the Metro graph. This is a replacement for
 * hiltViewModel() from Hilt.
 *
 * For ViewModels provided directly by the graph (no runtime params), use: val vm: MyViewModel =
 * metroViewModel()
 *
 * For ViewModels requiring assisted injection with runtime params, use: val vm: MyViewModel =
 * metroViewModel { pokedexViewModelFactory.create(param) }
 */
@Composable
    ...
    crossinline initializer: ApplicationGraph.(CreationExtras) -> T = { extras ->
        when (T::class) {
            ...
            PokedexViewModel::class ->
                pokedexViewModelFactory.create(extras.createSavedStateHandle())
            else ->
